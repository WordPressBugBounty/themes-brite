...
function brite_page_builder_components($components)
{
...
function brite_default_components($components)
{
...
function brite_register_components($components = array())
{
	if (apply_filters('colibri_page_builder/installed', false)) {
		$components = brite_page_builder_components($components);
	} else {
		$components = brite_default_components($components);
	}

	return $components;
}

Hooks::prefixed_add_action('components', 'brite_register_components');
Theme::load(array(
	'themeRelativePath'     => '',
 ...
 */
function brite_assets()
{
	return brite_theme()->getAssetsManager();
}
...
if (!apply_filters('colibri_page_builder/installed', false)) {
	brite_assets()
		...
		->registerStylesheet("brite-theme", "/theme/theme.css")
		->addGoogleFont("Open Sans", array("300", "400", "600", "700"))
		->addGoogleFont("Playfair Display", array("300", "400", "600", "700"))
		->addGoogleFont("Source Sans Pro", array("300", "400", "600", "700"))
		->addGoogleFont("Lato", array("300", "400", "600", "700"))
		->addGoogleFont("Montserrat", array("300", "400", "600", "700"))
		->addGoogleFont(
			"Muli",
			array(
				"300",
...
function brite_show_post_meta_setting_filter($value)
{
...
add_filter('colibri_show_post_meta', 'brite_show_post_meta_setting_filter');


function brite_posts_per_row_setting_filter($value)
{
...
add_filter('colibri_posts_per_row', 'brite_posts_per_row_setting_filter');

function brite_archive_post_highlight_setting_filter($value)
{
...
add_filter('colibri_archive_post_highlight', 'brite_archive_post_highlight_setting_filter');


function brite_blog_sidebar_enabled_setting_filter($value)
{
...
Hooks::prefixed_add_filter('blog_sidebar_enabled', 'brite_blog_sidebar_enabled_setting_filter');

function brite_override_with_thumbnail_image($value)
{
	global $post;

	if (isset($post) && $post->post_type === 'post') {
		$value = get_theme_mod(
			'blog_show_post_featured_image',
...
add_filter('colibri_override_with_thumbnail_image', 'brite_override_with_thumbnail_image');

function brite_print_archive_entry_class($class = "")
{
	...
	$postsPerRow = max(1, apply_filters('brite_posts_per_row', $default));


	$classes[] = "h-col-sm-12 h-col-md-" . (12 / intval($postsPerRow));

	$classes = apply_filters('brite_archive_entry_class', $classes);
...
function brite_print_masonry_col_class($echo = false)
{

	global $wp_query;
	$index        = $wp_query->current_post;
	$hasBigClass  = (is_sticky() || ($index === 0 && apply_filters('brite_archive_post_highlight', false)));
...
Hooks::prefixed_add_filter('info_page_tabs', 'brite_get_started_info_page_tab');

function brite_get_started_info_page_tab($tabs)
{
...
function brite_theme_plugins($plugins)
{
	$theme_plugins = array();
...
Hooks::prefixed_add_filter('theme_plugins', 'brite_theme_plugins');


add_filter('http_request_host_is_external', 'brite_allow_internal_host', 10, 3);
function brite_allow_internal_host($allow, $host, $url)
{
...
add_action('wp_ajax_brite_front_set_predesign', function () {
    check_ajax_referer( 'brite_front_set_predesign_nonce', 'nonce' );
...
function brite_override_main_row_class($classes)
{
	return Defaults::get('templates.blog.row.layout-classes', $classes);
}

Hooks::prefixed_add_filter('main_row_class', 'brite_override_main_row_class', 10, 1);
require_once __DIR__ . "/integration/colibri-page-builder/colibri-page-builder-integration.php";


add_action('widgets_init', 'brite_register_sidebars');

function brite_register_sidebars()
{
	register_sidebar(
		array(
			'name'          => esc_html__('Blog sidebar widget area', 'brite'),
	...
	if (function_exists('\is_shop')) {
		register_sidebar(
			array(
				'name'          => esc_html__('Woo Commerce left sidebar widget area', 'brite'),
				'id'            => 'brite-ecommerce-left',
				'before_widget' => '<div id="%1$s" class="widget %2$s">',
				'before_title'  => '<h5 class="widgettitle">',
				'after_title'   => '</h5>',
				'after_widget'  => '</div>',
