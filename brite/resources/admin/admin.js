...
// admin notice
(function ($) {
    var $notice_container = $('.brite-admin-big-notice--container');
    ...
    function disableNotice() {
        wp.ajax.post('brite_disable_big_notice', {
            nonce: brite_builder_status.brite_disable_big_notice_nonce
        });
    }

    function toggleProcessing(value) {
        $(window).on('beforeunload.brite-admin-big-notice', function () {
            return true;
        });
        if (value) {
            $('.brite-admin-big-notice').addClass('processing');
            $('.brite-admin-big-notice .action-buttons').fadeOut();
        } else {
            $('.brite-admin-big-notice').removeClass('processing');
        }
    }
    ...
    function installBuilder(callback) {
        pluginNotice(brite_builder_status.messages.installing);
        $.get(brite_builder_status.install_url).always(function () {
            toggleProcessing(true);
            activateBuilder(callback);
        }).always(function () {
            $(window).off('beforeunload.brite-admin-big-notice');
        });
    }

    function activateBuilder(callback) {
        pluginNotice(brite_builder_status.messages.activating);

        wp.ajax.post('brite_activate_plugin', {
            slug: brite_builder_status.slug,
            source: 'notice-' + selectedFrontPage,
            _wpnonce: brite_builder_status.colibri_plugin_install_activate_nonce
        }).done(function () {
            $(window).off('beforeunload.brite-admin-big-notice');
            if (callback) {
                callback();
    ...
    function processBuilderInstalationStepts(callback) {
        wp.ajax.post('brite_front_set_predesign', {
            index: selectedFrontPage,
            nonce: brite_builder_status.brite_front_set_predesign_nonce
        });
        if (brite_builder_status.status === "not-installed") {
            toggleProcessing(true);
            installBuilder(callback);
        }

        if (brite_builder_status.status === "installed") {
            toggleProcessing(true);
            activateBuilder(callback);
        }
    }
    ...
    $notice_container.on('click', '.view-all-demos', function () {
        ...
        processBuilderInstalationStepts(function () {
            window.location = brite_builder_status.view_demos_url;
        });
    });
    ...
    var briteInstallPluginSuccess = function briteInstallPluginSuccess(response) {
    ...
    var briteInstallPlugin = function briteInstallPlugin(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: briteInstallPluginSuccess
        });
    };

    var briteActivatePlugin = function briteActivatePlugin(event) {
        var $button = $(event.target);
    ...
    $document.on("click", ".colibri-plugin.install-now", briteInstallPlugin);
    $document.on("click", ".colibri-plugin.activate-now", briteActivatePlugin);
